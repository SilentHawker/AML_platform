...
import { listMasterPrompts, createMasterPrompt } from '../services/promptService';
import Spinner from './Spinner';
import { PlusIcon } from './icons/PlusIcon';

const PromptForm: React.FC<{ onSave: () => void, onCancel: () => void }> = ({ onSave, onCancel }) => {
    ...
    return (
        <div className="p-4 border rounded-md bg-gray-50 animate-fade-in-down mb-6">
            <h3 className="text-lg font-semibold mb-3">Create New Master Prompt</h3>
                ...
                <div>
                 ...
                 <div className="flex justify-end space-x-3">
                    ...
                    <button type="submit" disabled={isSubmitting} className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 disabled:bg-gray-400">
                        {isSubmitting ? <><Spinner/> Saving...</> : 'Save Prompt'}
                    </button>
                </div>
            </form>
        </div>
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                ...
                {!isFormVisible && (
                     <button onClick={() => setIsFormVisible(true)} className="inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700">
            ...
            {isFormVisible && <PromptForm onSave={handleSave} onCancel={() => setIsFormVisible(false)} />}
            
            {isLoading ? <div className="flex justify-center p-8"><Spinner /></div> : 
             error ? <p className="text-red-600 text-center p-4">{error}</p> : (
                <div className="space-y-3 pb-6">
                    {prompts.map(prompt => (
                        <div key={prompt.id} className="p-4 border rounded-md shadow-sm flex items-start justify-between">
                            <div>
